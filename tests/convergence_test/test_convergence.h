    ...
    void test_linear_regression_convergence() {
        ...
        try {
            ...
            for (int i = 0; i < n_samples; ++i) {
                float x = static_cast<float>(i) / (n_samples - 1);
                X_train(i, 0) = x;
    ...
    void test_binary_classification_convergence() {
        ...
        try {
            ...
            for (int i = 0; i < n_samples / 2; ++i) {
                X_train(i, 0) = (rand() % 100) / 200.0f;
                X_train(i, 1) = (rand() % 100) / 200.0f;
                Y_train(i, 0) = 0.0f;

                X_train(i + n_samples/2, 0) = 0.5f + (rand() % 100) / 200.0f;
                X_train(i + n_samples/2, 1) = 0.5f + (rand() % 100) / 200.0f;
